...
/*   Updated: 2023/07/19 09:19:17 by azari            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
unsigned int	get_colors(mlx_texture_t *img, unsigned int x, unsigned int y)
{
	unsigned int	*colour;
...
// 	while (steps-- >= 0)
// 	{
// 		mlx->txtr->offsety = fabs(p.y - p1->y) * ((mlx->tex->so->height / (p2->y - p1->y)));
		
// 		mlx_put_pixel(mlx->img, p.x, p.y,
// 			color(mlx->tex->so, mlx->txtr->offsetx, mlx->txtr->offsety));
// 		p.x += inc.x;
// 		p.y += inc.y;
// 	}
// }
...
void	render_wall(t_data *mlx, double dis, int x)
{
	...
	s->x = x;
	f->x = x;
	mlx->txtr->offsetx = fmod((x * (mlx->tex->ea->width / TILE_SIZE)), mlx->tex->ea->width);
